...
@section('content')
    ...
    <div class="panel panel-flat border-top-info">
            ...
            <div class="col-xs-5">
                <form action="{{ route('staff.index') }}" method="GET">
                    <div class="form-group input-group">
                        <input type="text" name="q" class="form-control" placeholder="Search staff" required/>
        ...
        {!! Form::open(['route' => 'staff.destroy','method'=>'DELETE','id'=>'formDelete']) !!}
            ...
            <table class="table table-hover">
                ...
                @forelse($staffs as $staff)
                    <tr>
                        <td>
                            <div class="pretty p-default">
                                {!! Form::checkbox('toDelete[]',$staff->id, false,['class'=>'checkItem']) !!}
                                <div class="state">
                                    <label></label>
                                </div>
                            </div>
                        ...
                        <td>{{ucwords($staff->phone)}}</td>
                        <td>{{ucwords($staff->address)}}</td>
                        <td>{{ucwords($staff->referred_by)}}</td>
                        <td>{{ucwords($staff->joined_date)}}</td>
